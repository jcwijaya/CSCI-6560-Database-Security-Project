    ...
    if (queryResults.length < 1 || queryResults.length > 2)
      ...
      // Check results- get role of requesting user and target user
        ...
        if (result.user_id === req.user.user_id) {
          ...
          if( actionEndpoint !== Action.REVOKE_BUCKET_ROLE.endpoint && 
            actionEndpoint !== Action.REVOKE_MAINTAINER_ROLE.endpoint &&
            BucketRole[result.bucket_role].string === req.body.targetRole ){
            console.log(`Target user has already been assigned the ${req.body.targetRole} for bucket ${req.body.bucket_name} `);
            throw new Error(`Target user has already been assigned the ${req.body.targetRole} for bucket ${req.body.bucket_name} `);
          }
          //For revoke role, if target user does not have the role we are trying to revoke, then return error too
          else if( (actionEndpoint === Action.REVOKE_BUCKET_ROLE.endpoint ||
            actionEndpoint === Action.REVOKE_MAINTAINER_ROLE.endpoint ) &&
            BucketRole[result.bucket_role].string !== req.body.targetRole ){
              let errorString = `Target user role ${BucketRole[result.bucket_role].string} found in database does not match role being revoked ${req.body.targetRole} for bucket ${req.body.bucket_name} `;
      ...
      switch (actionEndpoint) {
        ...
        case Action.UPDATE_BUCKET_ROLE.endpoint:
          ...
          console.log(`Attempt to add or modify user's role. Required role level is ${requiredRole}`);
          break;
        case Action.REVOKE_BUCKET_ROLE.endpoint:
            requiredRole = Action.REVOKE_BUCKET_ROLE.bucketRole.value;
            console.log(`Attempt to revoke user's role. Required role level is ${Action.REVOKE_BUCKET_ROLE.bucketRole.string}`);
          break;
        case Action.REVOKE_MAINTAINER_ROLE.endpoint:
          requiredRole = Action.REVOKE_MAINTAINER_ROLE.bucketRole.value;
          console.log(`Attempt to revoke maintainer's role. Required role level is ${Action.REVOKE_MAINTAINER_ROLE.bucketRole.string}`);
          break;
      }
